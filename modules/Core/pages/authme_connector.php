            ...
            if ($captcha_passed) {
                ...
                if ($validation->passed()) {
                    ...
                    if ($authme_conn->connect_errno) {
                        ...
                        $stmt = $authme_conn->prepare('SELECT password, address FROM ' . $authme_db['table'] . ' WHERE realname = ?');
                        if ($stmt) {
                            ...
                            if (is_null($password)) {
                                ...
                                switch ($authme_db['hash']) {
                                    ...
                                    case 'sha256':
                                        ...
                                        if ($salt . hash('sha256', hash('sha256', $_POST['password']) . $salt) == $salt . $exploded[2]) {
                                            $valid = true;
                                            $_SESSION['authme'] = [
                                                'user' => Input::get('username'),
                                    ...
                                    case 'sha512':
                                        ...
                                        if ($salt . hash('sha512', hash('sha512', $_POST['password']) . $salt) == $salt . $exploded[2]) {
                                            $valid = true;
                                            $_SESSION['authme'] = [
                                                'user' => Input::get('username'),
                                                'pass' => ($salt . '$' . $exploded[3]),
